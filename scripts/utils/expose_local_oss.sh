...
NAMESPACE=spinnaker-oss

BASE_DIR=/etc/spinnaker

for SVC in front50 igor rosco echo deck orca gate kayenta fiat clouddriver redis; do
  touch ${BASE_DIR}/.haloss/default/service-settings/${SVC}.yml
  yq w -i ${BASE_DIR}/.haloss/default/service-settings/${SVC}.yml kubernetes.serviceType LoadBalancer
done
